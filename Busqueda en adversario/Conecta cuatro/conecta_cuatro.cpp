...
void cpyBoard(char to[6][7], char from[6][7]){
  for(int i = 0; i < 6; i++){
    for(int j = 0; j < 7; j++){
      to[i][j] = from[i][j];
    }
...
int minfunction(char board[6][7], int posix[], int nivel){
...
int maxfunction(char board[6][7], int posix[], int nivel){
  ...
  if(nivel == 0 || endgame != 3){
    ...
    return evalFunction(board, 'x') - evalFunction(board, 'o');
  }
  for(int i = 0; i < 7; i++){
    if(posix[i] >= 0){
      board[posix[i]][i] = 'o';
...
int main(){
  ...
  do{
    ...
    cpyBoard(cpyOfBoard, board);
    tiro = maxfunction(cpyOfBoard, posix, 8);
