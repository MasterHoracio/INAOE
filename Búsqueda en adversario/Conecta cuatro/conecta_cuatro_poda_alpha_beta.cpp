...
int maxfunction(char [6][7], int [], int, int, int);
int minfunction(char [6][7], int [], int, int, int);
...
int minfunction(char board[6][7], int posix[], int nivel, int alpha, int beta){//juega x
  ...
  for(int i = 0; i < 7; i++){
    if(posix[i] >= 0){
      ...
      posix[i]--;
      ++nodos_generados;
      v = maxfunction(board, posix, nivel - 1, alpha, beta);
      posix[i]++;
      board[posix[i]][i] = '-';
      if(v < local_min){
        local_min = v;
...
int maxfunction(char board[6][7], int posix[], int nivel, int alpha, int beta){//juega o
  ...
  for(int i = 0; i < 7; i++){
    if(posix[i] >= 0){
      ...
      posix[i]--;
      ++nodos_generados;
      v = minfunction(board, posix, nivel - 1, alpha, beta);
      posix[i]++;
      board[posix[i]][i] = '-';
      if(v > local_max){
        local_max = v;
...
int main(){
  ...
  cout << endl << "Conecta cuatro con Poda Alpha-Beta" << endl << endl;
  printBoard(board);
  ...
  do{
    ...
    nodos_generados = 0;
    tiro = maxfunction(board, posix, 8, MENOS_INFINITO, MAS_INFINITO);
